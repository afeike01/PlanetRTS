...
public class NodeCluster
{
    ...
    public void StorePath(Node startNode, Node endNode, bool refresh = false)
    {
        if (!startNode.IsTemporary() && !endNode.IsTemporary())
        {
            ...
            else if (storedPathsDictionary.ContainsKey(connectionKey) && newPath != null && refresh)
            {
    ...
    public void RefreshPaths(/*Node newNode*/)
    {
        ...
        for (int i = 0; i < nodeList.Count; i++)
        {
            ...
            for (int j = 0; j < nodeList.Count; j++)
            {
                ...
                StorePath(startNode, endNode, true);
